...
public class CodesSecurityUtils extends CommonSecurityUtils {
    ...
    public static void canResendCodelist(ServiceContext ctx) throws MetamacException {
        if (!SharedCodesSecurityUtils.canResendCodelistStreamMessage(getMetamacPrincipal(ctx))) {
            throwExceptionIfOperationNotAllowed(ctx);
        }
    }
