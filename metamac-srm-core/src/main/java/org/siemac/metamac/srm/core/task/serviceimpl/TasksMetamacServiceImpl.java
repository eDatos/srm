...
public class TasksMetamacServiceImpl extends TasksMetamacServiceImplBase {
    ...
    public synchronized String plannifyPublicationInternallyCodelist(ServiceContext ctx, String urn, Boolean forceLatestFinal) throws MetamacException {
        try {
            ...
            // @formatter:off
            JobDetail job = newJob(MergeCodelistJob.class).withIdentity(jobKey, "merge")
                                                          .usingJobData(MergeCodelistJob.USER, ctx.getUserId())
                                                          .usingJobData(MergeCodelistJob.URN, urn)
                                                          .usingJobData(MergeCodelistJob.FORCE_LATEST_FINAL, forceLatestFinal)
                                                          .requestRecovery()
                                                          .build();
            // @formatter:on
