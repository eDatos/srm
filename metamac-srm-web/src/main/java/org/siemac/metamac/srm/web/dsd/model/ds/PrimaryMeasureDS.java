package org.siemac.metamac.srm.web.dsd.model.ds;

public class PrimaryMeasureDS {

    public static final String ID                                     = "pm-id";
    public static final String CODE                                   = "pm-code";
    public static final String URN                                    = "pm-urn";
    public static final String URN_PROVIDER                           = "pm-urn-provider";

    public static final String CONCEPT                                = "pm-con";
    public static final String CONCEPT_EDITION_VIEW                   = "pm-con-ed-view";   // Not mapped in DTO
    public static final String CONCEPT_VIEW                           = "pm-con-view";      // Not mapped in DTO

    public static final String REPRESENTATION_TYPE                    = "pm-repr-type";
    public static final String REPRESENTATION_TYPE_VIEW               = "pm-repr-type-view"; // Not mapped in DTO

    public static final String ENUMERATED_REPRESENTATION              = "pm-repr-enum";
    public static final String ENUMERATED_REPRESENTATION_EDITION_VIEW = "pm-repr-enum-ed";  // Not mapped in DTO
    public static final String ENUMERATED_REPRESENTATION_VIEW         = "pm-repr-enum-view"; // Not mapped in DTO
}
